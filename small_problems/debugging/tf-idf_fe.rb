...
def tfidf(term, document, documents)
  result = (tf(term, document) * idf(term, documents)).round(1)
  tf(term, document) == 0 ? 0 : result # code added here
end

def tf(term, document)
  document.split(/[\s,.-]/).count do |word|
    word.downcase == term
  end
end
...
def idf(term, documents)
  ...
  return 0 if number_of_documents_with_term == 0 # code added here
...
documents = [document2, document3] # document1 remove from documents array
