...
an Array that contains every word from the string,
...
#   words = string.split(' ')
#   how_many = words.map { |str| str.length }
#   words.zip(how_many).map { |word| word.join(' ') }
# end
...
# another alt
def word_lengths(string)
  string.split(' ').map {|word| word = word + " #{word.size}" }
end
